...
public class OrderForm extends JFrame implements ActionListener
{
	...
	private JButton btnAdd, btnExit, btnComp, btnQ, btnStat ;
	...
	private JPanel Order, pSize, Banner, banLeft, banMid, banRight, UI, Queue, pOpt;
	...
	private boolean qVis = false, statVis = false;
	private File out;
	...
	Font all = new Font("Courier", Font.BOLD,14); // Create a font to use as default
	...
	public OrderForm() 
	{
		// construct containers and panels
		Main = getContentPane ();
		Main.setLayout(new GridBagLayout());
		Main.setBackground(Color.WHITE);

		Order = new JPanel(new GridBagLayout());
		...
		pSize= new JPanel(new GridBagLayout());	

		pOpt= new JPanel(new GridBagLayout());
		...
		// Creating Box and layout for the CheckBox
		...
		// Create the components for the banner and setting colours
		banLeft = new JPanel();
		...
		fill1 = new JLabel("",JLabel.CENTER); // blank labels to fill space in banner
		fill2 = new JLabel("",JLabel.CENTER);

		// Buffer Labels to help with spacing and sizing of GridBag
		...
		// Creating labels for the order panel and pOpt Panel
		...
		lblnext = new JLabel("<html><u>--Next Pizza--</u></html>",JLabel.CENTER); // label for next pizza text field

		lblStats = new JLabel("<html><u>--Statistics--</u></html>",JLabel.CENTER);// label for Statistics text field
		lblStats.setVisible(false);
		pic = new JLabel(icon); // Label to hold picture displayed when the queue is hidden
		...
		queue = new JTextArea();
		queue.setFont(new Font("monospaced", Font.PLAIN, 12));
		...
		addComp(Queue,pic,0,0,1,1,0,0);
		addComp(Queue,jsp,0,0,4,1,1,1);
		...
		// adding components to pOpt panel
		...
		// adding components to Order pane;
		...
		// adding components to pSize panel
		...
		Read(); // read in saved orders from existing Orders.dat file

		if (Orders.empty()== false) // update the next pizza text field with new orders from Orders.dat file
		{
			next();
		}
	...
	{		
		...
		if(e.getSource() == btnComp)
		{
			...
			else
			{
				...
				next.setText("");
				String Header; 
				Header = String.format("%-11s|%-20s|%-16s|%-15s|%-60s|%-7s|\n", 
						"Order", "Date/Time", "Size", "Crust", "Toppings", "Price");
				next.append(Header);
				Header="---------------------------------------------------------------------"
						+ "------------------------------------------------------------------\n";
	...
	public void AddOrder()
	{
		...
		else
		{
			...
			if(newOrder.getpBase().equals("Deep Pan"))
			{
				DeepNo++;
			}
			else
			...
			if(newOrder.getpSize().equals("Small"))
			{
				sNo++;
			}
			else if(newOrder.getpSize().equals("Medium"))
			{
				mNo++;
			}
			else if(newOrder.getpSize().equals("Large"))
			{
				lNo++;
			}
			else
			...
			if(newOrder.getpTop().toLowerCase().contains("mushrooms"))
			{
				MuNo++;
			}

			if(newOrder.getpTop().toLowerCase().contains("bacon"))
			{
				BaNo++;
			}
			
			if(newOrder.getpTop().toLowerCase().contains("chicken"))
			{
				ChNo++;
			}
			
			if(newOrder.getpTop().toLowerCase().contains("pepperoni"))
			{
				PeNo++;
			}
			
			if(newOrder.getpTop().toLowerCase().contains("pineapple"))
			{
				PiNo++;
			}
			
			if(newOrder.getpTop().toLowerCase().contains("olives"))
			{
				OlNo++;
			}
	...
	{ 
		...
		stats.append("   --Total Orders & Income--\n"
				+"---------------------------------\n"
				+ "  Total Orders\t|  Total Income\t|\n"		
				+ "---------------------------------\n"
				+ "\t"+totalOrders+"\t|\t£"+df2.format(totalIncome)+"\t|\n" 
				+ "---------------------------------\n\n");

		stats.append(" --Base Type Order Statistics--\n"
				+"---------------------------------\n"
				+ "  Deep Pan\t|  Thin Crust\t|\n"
				+"---------------------------------\n   "
				+ "\t"+DeepNo+"\t|\t"+ThinNo+"\t|\n"
		...
		stats.append("         --Pizza Size Order Statistics--\n"
				+"-------------------------------------------------\n"
	...
	public void ShowStats()
	{
		if(statVis == false)
		{
			...
			stats.setText("");
			Stats();
			btnStat.setText("Hide Statistics");
		}
		else
		{
			statVis = false;
			...
			btnStat.setText("Show Statistics");
		}
